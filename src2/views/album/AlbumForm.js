...
const AlbumForm = ({ className, create, status, entity, ...rest }) => {
  ...
  const formik = useFormik({
    initialValues: {
      name: "", type: "ALBUM", artist: null, releaseDate: null,
    },
    onSubmit: values => {
      create(values, {
        onSuccess: d => {
          setMessage(`New Album "${d.name}" by ${d.artist.name}`)
          setOpen(true)
          navigate(`/app/albums/${d.uuid}`,{replace: true})
        },
        onFail: e => {
          console.log(e)
        }
  ...
  return (
    ...
    >
      <Card>
        <CardHeader
          subheader="Enter all requires information about the new on album on the platform"

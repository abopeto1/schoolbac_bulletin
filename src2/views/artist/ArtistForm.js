...
const ArtistForm = ({ className, create, status, entity, ...rest }) => {
  ...
  const formik = useFormik({
    ...
    onSubmit: values => {
      create(values, {
        onSuccess: () => {
          setMessage(`New Artist created`)
          setOpen(true)
          navigate(`/app/artists`,{replace: true})
        },
        onFail: e => {
          console.log(e)
        }
  ...
  return (
    ...
    >
      <Card>
        <CardHeader
          subheader="Enter all requires information about the new Artist on the platform"
          title="New Artist"
        />
        <Divider />
        <CardContent>
        ...
        >
          ...
          >
            Save {upperFirst(rest.entityName)}
          </Button>
        </Box>
      </Card>
      <SnackBar
